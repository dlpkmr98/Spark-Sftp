...
class MultiFileReader[R<:Formatter](r: R) extends Transformation[Seq[String], Seq[DataFrame]] with IO {
 ...
 */
  override def execute = x => for (path <- x) yield {
     r.format match {
       case "csv" => toReadDataFromCsv(path)
       case "parquet" => toReadDataFromParquet(path)
